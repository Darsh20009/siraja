...
export interface ListAyahPerformanceQuery {
  surahNumber?: number;
  heatmapLevel?: HeatmapLevel;
}

@Injectable()
export class ListAyahPerformanceUseCase {
  constructor(
  ...
  async execute(user: AccessTokenPayload, studentId: string, query: ListAyahPerformanceQuery) {
    ...
    const [items, summary] = await Promise.all([
      this.performanceRepo.findByStudent(user.tenantId, studentId, query),
      this.performanceRepo.getSummary(user.tenantId, studentId, query.surahNumber),
    ]);
