...
/**
 * GetHeatmapUseCase — Memorization Heatmap (Phase 9). No schema of its
 * own; reads the stored `heatmapLevel` field already computed on
 * `AyahPerformance` records at write time.
 */
@Injectable()
export class GetHeatmapUseCase {
  ...
  async execute(user: AccessTokenPayload, studentId: string, surahNumber?: number) {
    ...
    const [ayahs, summary] = await Promise.all([
      this.performanceRepo.findByStudent(user.tenantId, studentId, { surahNumber }),
      this.performanceRepo.getSummary(user.tenantId, studentId, surahNumber),
    ]);
